...
const CheckoutModals = (props: CheckoutProps) => {
  ...
  return (
    ...
    <div>
      {currentState === 0 && (
        <div className="text-white-100 w-full">
          <div className="w-full bg-black-100 rounded-3xl p-[24px] sm:p-[16px] flex flex-col gap-4 sm:gap-3">
            ...
            <button onClick={() => setCurrentState(1)} className="w-[100%] bg-primary-100 font-SFProMedium font-normal py-[8px] rounded-[100px] text-base">
              Pay now
            </button>
          </div>
        </div>
      ...
      {
        currentState === 1 && (
          <Loading text='Complete the payment using your wallet'/>
        )
      }
      {
        currentState === 2 && (
          <CheckoutSuccess name={"Notre-Dame, Floods (1910) "} author={"Albert Marquet"} status={"Processing"} img={image}/>
        )
      }
      {
        currentState === 3 && (
          <CheckoutSuccess name={"Notre-Dame, Floods (1910) "} author={"Albert Marquet."} status={"Successful"} img={image} handleOpenCheckOutModal={handleOpenCheckOutModal}/>
        )
      }
    </div>
  );
